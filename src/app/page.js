...
export default function GeneralChat() {
  ...
  return (
    <div className={`py-8 px-5 sm:px-14 lg:px-72 mx-auto h-screen ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      <HeaderBar title="Chatbot Pro AI (Temporary)" darkMode={darkMode} toggleDarkMode={() => setDarkMode(!darkMode)}>
        <Link href="/summarizer" className={`text-sm underline ${darkMode ? 'text-white' : 'text-amber-600'}`}>AI Summarizer</Link>
      </HeaderBar>
